...
var
  i: integer;
begin
  i := AGXSceneObject.behaviours.IndexOfClass(TgxBInertia);
  if i >= 0 then
    Result := TgxBInertia(AGXSceneObject.behaviours[i])
  else
    Result := nil;
end;
...
procedure TgxBInertia.ApplyForce(const deltaTime: double; const force: TVector);
begin
  if Mass <> 0 then
...
procedure TgxBInertia.SurfaceBounce(const surfaceNormal: TVector; restitution: single);
var
  f: single;
begin
