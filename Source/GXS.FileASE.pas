...
type
  ...
  private
    ...
    FSmoothing: TgxASESmoothingGroups;
    FSubMaterialID: Integer;                    // submaterial ID
    FTextChannels: TgxASEFaceTexureChannels;
  public
    constructor Create;
    ...
    property TextChannels: TgxASEFaceTexureChannels read FTextChannels; // channels texture coordinates
    property Smoothing: TgxASESmoothingGroups read FSmoothing;   // smoothing group list
  ...
  private
    FItems: TList;
    function GetFace(Index: Integer): TgxASEFace;
    function GetCount: Integer;
  public
    constructor Create;
    destructor Destroy; override;
    function Add: TgxASEFace;
    procedure Delete(aIndex: Integer);
    procedure Clear;
    property Face[Index: Integer]: TgxASEFace read GetFace; default;
  ...
  TgxASEMaterialTextureMap = record
    Kind: string;
    Name: string;
    _Class: string;
    No: Integer;
  ...
  TgxASEMaterialTextureMaps = record
    Map: array [0..GL_ASE_MAX_TEXTURE_MAPS - 1] of TgxASEMaterialTextureMap;
    Count: Integer;
  end;

  TgxASESubMaterial = record
    Name: string;
    ...
    TextureMaps: TgxASEMaterialTextureMaps;
  end;

  TgxASESubMaterialList = record
    SubMaterial: array [0..GL_ASE_MAX_SUBMATERIALS - 1] of TgxASESubMaterial;
    Count: Integer;
  end;

  TgxASEMaterial = class(TObject)
  private
    FWireSize: Single;
    ...
    FSubMaterials: TgxASESubMaterialList;
    FTextureMaps: TgxASEMaterialTextureMaps;
  public
    constructor Create;
    ...
    property TextureMaps: TgxASEMaterialTextureMaps read FTextureMaps;
    property SubMaterials: TgxASESubMaterialList read FSubMaterials;
  end;

  TgxASEMaterialList = class(TObject)
  private
    FItems: TList;
    function GetCount: Integer;
    function GetMaterial(Index: Integer): TgxASEMaterial;
  public
    constructor Create;
    destructor Destroy; override;
    function Add: TgxASEMaterial;
    procedure Delete(aIndex: Integer);
    procedure Clear;
    property Material[Index: Integer]: TgxASEMaterial read GetMaterial; default;
    property Count: Integer read GetCount;
  end;
  ...
  private
    ...
    FCastShadow: Boolean;
    FMotionBlur: Boolean;
    FMaterialList: TgxASEMaterialList;
    ...
    procedure ParseFaceString(const aData: string; var Index, A, B, C, AB, BC, CA, MatID: Integer; var Smooth: TgxASESmoothingGroups);
  private
    procedure ParseScene(var aLineIndex: Integer);
    procedure ParseGeomObject(var aLineIndex: Integer);
    procedure ParseMeshOptions(var aLineIndex: Integer; aMesh: TgxASEMeshObject);
    procedure ParseMeshGeom(var aLineIndex: Integer; aMesh: TgxASEMeshObject);
    procedure ParseMappingChannel(var aLineIndex: Integer; aMesh: TgxASEMeshObject);
    procedure ParseMeshVertices(var aLineIndex: Integer; aMesh: TgxASEMeshObject; VerticesCount: Integer);
    procedure ParseMeshFaces(var aLineIndex: Integer; aMesh: TgxASEMeshObject; FacesCount: Integer);
    procedure ParseMeshNormals(var aLineIndex: Integer; aMesh: TgxASEMeshObject; FacesCount: Integer);
    procedure ParseMeshTextureVertices(var aLineIndex: Integer; aMesh: TgxASEMeshObject; TextureVerticesCount: Integer);
    procedure ParseMeshTextureFaces(var aLineIndex: Integer; aMesh: TgxASEMeshObject; TextureFacesCount: Integer);
    procedure ParseMaterialList(var aLineIndex: Integer);
    procedure ParseMaterial(var aLineIndex: Integer; aMaterial: TgxASEMaterial);
    procedure ParseSubMaterial(var aLineIndex: Integer; aMaterial: TgxASEMaterial);
    function CheckTextureMap(var aLineIndex: Integer; var aMaps: TgxASEMaterialTextureMaps): Boolean;
    procedure ParseTextureMap(var aLineIndex: Integer; var aMaps: TgxASEMaterialTextureMaps; const aMapKind: string);
...
procedure CopyASEToMesh(aASEMesh: TgxASEMeshObject; aMesh: TgxMeshObject; aASEMaterials: TgxASEMaterialList);
  ...
  function FindTextureMap(aMaterial: TgxASEMaterial; aMapKind: string; aSubmaterialIndex: Integer;
    out TM: TgxASEMaterialTextureMap): Boolean;

    function FindInMaps(const aMaps: TgxASEMaterialTextureMaps): Boolean;
    var
      i: Integer;
    begin
      Result := False;
...
constructor TgxASEFaceList.Create;
begin
  FItems := TList.Create;
end;

destructor TgxASEFaceList.Destroy;
begin
  Clear;
  FreeAndNil(FItems);
  inherited;
...
function TgxASEFaceList.Add: TgxASEFace;
begin
  Result := TgxASEFace.Create;
  FItems.Add(Result);
end;

procedure TgxASEFaceList.Delete(aIndex: Integer);
begin
  Face[aIndex].Free;
  FItems.Delete(aIndex);
end;

procedure TgxASEFaceList.Clear;
var
  i: Integer;
begin
...
function TgxASEFaceList.GetFace(Index: Integer): TgxASEFace;
begin
  Result := FItems[Index];
end;

function TgxASEFaceList.GetCount: Integer;
begin
  Result := FItems.Count;
end;
...
constructor TgxASEMaterial.Create;
begin
...
destructor TgxASEMeshObject.Destroy;
var
  i: Integer;
begin
  FreeAndNil(FFaces);
...
function TgxASEMeshObject.AddTexChannel: TAffineVectorList;
begin
...
function TgxASEMeshObject.GetTextChannel(Channel: Integer): TAffineVectorList;
begin
...
constructor TgxASEMaterialList.Create;
begin
  FItems := TList.Create;
end;

destructor TgxASEMaterialList.Destroy;
begin
  Clear;
  FreeAndNil(FItems);
  inherited;
...
function TgxASEMaterialList.Add: TgxASEMaterial;
begin
  Result := TgxASEMaterial.Create;
  FItems.Add(Result);
end;

procedure TgxASEMaterialList.Delete(aIndex: Integer);
begin
  Material[aIndex].Free;
  FItems.Delete(aIndex);
end;

procedure TgxASEMaterialList.Clear;
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
...
function TgxASEMaterialList.GetMaterial(Index: Integer): TgxASEMaterial;
begin
  Result := FItems[Index];
end;

function TgxASEMaterialList.GetCount: Integer;
begin
  Result := FItems.Count;
end;
...
constructor TgxASEVectorFile.Create;
begin
  inherited;
  FStringData := TStringList.Create;
  FMaterialList := TgxASEMaterialList.Create;
end;

destructor TgxASEVectorFile.Destroy;
begin
  FMaterialList.Free;
  FStringData.Free;
  inherited;
...
procedure TgxASEVectorFile.LoadFromStream(aStream: TStream);
begin
  FStringData.LoadFromStream(aStream);
  Parse;
end;

function TgxASEVectorFile.ContainString(const aData, aString: string): Boolean;
begin
  Result := (Length(aData) >= Length(aString)) and (Pos(AString, aData) > 0);
end;

function TgxASEVectorFile.GetDoubleValue(const aData: string): Double;
var
  Start: Integer;
  Data: string;
  Value: string;
...
function TgxASEVectorFile.GetEndOfFirstValue(const aData: string): Integer;
var
  Value: string;
begin
...
function TgxASEVectorFile.GetFirstValue(aData: string): string;
const
  sep1 = #32;
var
  Index: Integer;
...
function TgxASEVectorFile.GetPropCastShadow(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TgxASEVectorFile.GetPropMBlur(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TgxASEVectorFile.GetPropRECVShadow(const aData: string): Boolean;
begin
  Result := Pos('1', aData) > 0;
end;

function TgxASEVectorFile.GetStringValue(const aData: string): string;
const
  Breaker = '"';
var
  Start: Integer;
...
function TgxASEVectorFile.GetTagOnData(const aData: string): Integer;
var
  Index: Integer;
begin
  Result := -1;
...
function TgxASEVectorFile.GetValue3D(const aData: string): TAffineVector;
var
  Start: Integer;
  Data: string;
  Value: String;
...
function TgxASEVectorFile.GetValue4D(const aData: string; var Value0: Integer): TAffineVector;
var
  Start: Integer;
  Data: string;
  Value: String;
...
function TgxASEVectorFile.IsEndOfSection(const aData: string): Boolean;
begin
...
function TgxASEVectorFile.IsSectionBegingin(const aData: string): Boolean;
begin
  Result := Pos('{', aData) > 0;
end;

function TgxASEVectorFile.CheckUnknownData(var aLineIndex: Integer): Boolean;
begin
...
procedure TgxASEVectorFile.SkipSection(var aLineIndex: Integer);
var
  Data: string;
begin
  Inc(aLineIndex);
...
procedure TgxASEVectorFile.Parse;
var
  LineIndex: Integer;
  Data: string;
  b: Boolean;
...
procedure TgxASEVectorFile.ParseFaceString(const aData: string; var Index, A, B, C, AB, BC, CA, MatID: Integer;
  var Smooth: TgxASESmoothingGroups);
var
  Data: string;
  Start: Integer;
  Value: string;
  ...
  procedure GetNextValues(var aValues: TgxASESmoothingGroups);
...
procedure TgxASEVectorFile.ParseMeshFaces(var aLineIndex: Integer; aMesh: TgxASEMeshObject; FacesCount: Integer);
var
  Data: string;
  Index, A, B, C, AB, BC, CA, ID: Integer;
  face: TgxASEFace;
  smooth: TgxASESmoothingGroups;
begin
  Inc(aLineIndex);
  Data := FStringData[aLineIndex];
  while not IsEndOfSection(Data) do begin
...
procedure TgxASEVectorFile.ParseMeshNormals(var aLineIndex: Integer; aMesh: TgxASEMeshObject; FacesCount: Integer);
var
  Data: string;
  Index: Integer;
  Counter: Integer;
...
procedure TgxASEVectorFile.ParseMeshTextureFaces(var aLineIndex: Integer; aMesh: TgxASEMeshObject;
...
procedure TgxASEVectorFile.ParseMeshTextureVertices(var aLineIndex: Integer; aMesh: TgxASEMeshObject;
...
procedure TgxASEVectorFile.ParseMeshGeom(var aLineIndex: Integer; aMesh: TgxASEMeshObject);
var
  Data: string;
...
procedure TgxASEVectorFile.ParseMeshVertices(var aLineIndex: Integer; aMesh: TgxASEMeshObject;
  VerticesCount: Integer);
var
  Data: string;
  VertIndex: Integer;
...
procedure TgxASEVectorFile.ParseMappingChannel(var aLineIndex: Integer; aMesh: TgxASEMeshObject);
var
  Data: string;
  TextureVerticesCount: Integer;
  TextureFacesCount: Integer;
...
procedure TgxASEVectorFile.ParseMeshOptions(var aLineIndex: Integer; aMesh: TgxASEMeshObject);
var
  Data: string;
  b: Boolean;
begin
...
procedure TgxASEVectorFile.ParseScene(var aLineIndex: Integer);
begin
  CheckUnknownData(aLineIndex);
end;

procedure TgxASEVectorFile.ParseMaterialList(var aLineIndex: Integer);
var
  Data: string;
  material: TgxASEMaterial;
begin
...
function TgxASEVectorFile.CheckTextureMap(var aLineIndex: Integer; var aMaps: TgxASEMaterialTextureMaps): Boolean;
var
  Data: string;
begin
  Result := True;
...
procedure TgxASEVectorFile.ParseMaterial(var aLineIndex: Integer; aMaterial: TgxASEMaterial);
var
  Data: string;
  b: Boolean;
begin
...
procedure TgxASEVectorFile.ParseSubMaterial(var aLineIndex: Integer; aMaterial: TgxASEMaterial);
var
  Data: string;
  Index: Integer;
  b: Boolean;
...
procedure TgxASEVectorFile.ParseTextureMap(var aLineIndex: Integer; var aMaps: TgxASEMaterialTextureMaps; const aMapKind: string);
var
  Data: string;
  Index: Integer;
  b: Boolean;
