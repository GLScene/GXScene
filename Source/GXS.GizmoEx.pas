...
function TgxGizmoEx.MouseWorldPos(const X, Y: Integer): TVector;
var
  v: TVector;
  InvertedY: Integer;
begin
...
procedure TgxGizmoEx.ActivatingElements(PickList: TgxPickList);
  ...
  procedure ActlightRotateLine(const line: TgxLines; const dark: TVector);
  var
    v: TVector4f;
    I: Integer;
  begin
  ...
  procedure DeActlightRotateLine(const line: TgxLines; const dark: TVector);
  var
    v: TVector4f;
    I: Integer;
  begin
  ...
  procedure DeActlightLine(const line: TgxLines; const dark: TVector; alterStyle: Boolean = False);
  begin
  ...
  procedure ActlightRotateArrowLine(const line: TgxLines; Color: TVector);
  begin
    line.LineColor.color := Color;
    line.Options := [];
  end;

  procedure DeActlightRotateArrowLine(const line: TgxLines; const dark: TVector);
  begin
  ...
  procedure DeActlightText(const FlatText: TgxFlatText; const dark: TVector);
  begin
  ...
  procedure ActlightTextRotate(const FlatText: TgxFlatText; Color: TVector);
  begin
    FlatText.ModulateColor.Color := Color;
  end;

  procedure DeActlightTextRotate(const FlatText: TgxFlatText; const dark: TVector);
  begin
    with FlatText.ModulateColor do
...
procedure TgxGizmoEx.ViewerMouseDown(const X, Y: Integer);
  ...
  function SetInitialDiskPostition(aObject, aObject2: TgxCustomSceneObject): TVector;
  var
    rayStart, rayVector, iPoint, iNormal: TVector;
  begin
    if (Viewer = nil) then
...
procedure TgxGizmoExObjectItem.SetOldMatrix(const Value: TMatrix);
begin
  FOldMatrix := Value;
end;
