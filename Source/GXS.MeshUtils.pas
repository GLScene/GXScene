...
procedure ConvertStripToList(const strip: TAffineVectorList;
  const indices: TIntegerList; list: TAffineVectorList); overload;
...
procedure ConvertIndexedListToList(const data: TAffineVectorList;
  const indices: TIntegerList; list: TAffineVectorList);
...
function BuildNormals(reference: TAffineVectorList; indices: TIntegerList)
  : TAffineVectorList;
...
function BuildNonOrientedEdgesList(triangleIndices: TIntegerList;
  triangleEdges: TIntegerList = nil; edgesTriangles: TIntegerList = nil)
  : TIntegerList;
...
procedure WeldVertices(vertices: TAffineVectorList; indicesMap: TIntegerList;
  weldRadius: Single);
...
procedure SubdivideTriangles(smoothFactor: Single; vertices: TAffineVectorList;
  triangleIndices: TIntegerList; normals: TAffineVectorList = nil;
...
function MakeTriangleAdjacencyList(const AindicesList: PLongWordArray;
  Count: LongWord; const AVerticesList: PAffineVectorArray): TLongWordList;
...
procedure ConvertStripToList(const strip: TIntegerList; list: TIntegerList);
var
  i: Integer;
...
procedure RemapReferences(reference: TIntegerList; const indices: TIntegerList);
var
  i, n: Integer;
  tag: array of Byte;
...
begin
  ...
  boundaryList[b0].maxSqArea := GXS.MeshUtils.polygonArea(b0);
...
begin
  ...
  begin
    ...
    boundaryList[0].maxSqArea := GXS.MeshUtils.polygonArea(0);
    maxMaxSqArea := boundaryList[0].maxSqArea;

    for i := 1 to Count - 1 do
    begin
      boundaryList[i].maxSqArea := GXS.MeshUtils.polygonArea(i);
