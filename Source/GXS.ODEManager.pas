...
function GetBodyFromGXSceneObject(anObject: TgxBaseSceneObject): PdxBody;
...
procedure RegisterGXSceneObject(anObject: TgxBaseSceneObject);
procedure UnregisterGXSceneObject(anObject: TgxBaseSceneObject);
function GetGXSceneObject(anObjectName: String): TgxBaseSceneObject;
...
begin
  ...
  if Assigned(FOwnerBaseSceneObject) then
    RegisterGXSceneObject(OwnerBaseSceneObject);
end;
...
begin
  if Assigned(Manager) then
    Manager := nil;
  if Assigned(FOwnerBaseSceneObject) then
    UnregisterGXSceneObject(FOwnerBaseSceneObject);
  FSurface.Free;
  inherited;
end;
...
function TgxODEBehaviour.GetAbsoluteMatrix: TMatrix;
begin
...
procedure TgxODEDynamic.AlignBodyToMatrix(Mat: TMatrix);
var
  R: TdMatrix3;
begin
...
function TgxODEElementBase.AbsoluteMatrix: TMatrix;
var
  Mat: TMatrix;
begin
...
procedure TgxODEElementBase.AlignGeomElementToMatrix(Mat: TMatrix);
var
  R: TdMatrix3;
begin
...
function TgxODEElementBase.GetMatrix: TMatrix;
begin
  Result := FLocalMatrix;
end;
...
procedure TgxODEElementBase.SetMatrix(const Value: TMatrix);
begin
  FLocalMatrix := Value;
  RebuildVectors;
  ODERebuild;
...
procedure TgxODEElementTriMesh.SetVertices(const Value: TAffineVectorList);
begin
  FVertices.Assign(Value);
  RefreshTriMeshData;
end;

procedure TgxODEElementTriMesh.SetIndices(const Value: TIntegerList);
begin
...
procedure TgxODEElementPlane.AlignGeomElementToMatrix(Mat: TMatrix);
var
  d: Single;
begin
  if not Assigned(FGeomElement) then
...
begin
  ...
  if Enabled then
  begin
    Body1 := GetBodyFromGXSceneObject(FObject1);
    Body2 := GetBodyFromGXSceneObject(FObject2);
  end
  else
  begin
    Body1 := nil;
...
begin
  ...
  begin
    Obj := GetGXSceneObject(FObject1Name);
    if Assigned(Obj) then
      Object1 := Obj;
    FObject1Name := '';
  end;
  ...
  begin
    Obj := GetGXSceneObject(FObject2Name);
    if Assigned(Obj) then
      Object2 := Obj;
    FObject2Name := '';
  end;
...
procedure TgxODECustomCollider.SetTransform(ATransform: TMatrix);
begin
