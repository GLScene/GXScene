...
function TgxDummyCube.AxisAlignedDimensionsUnscaled: TVector;
begin
...
function TgxDummyCube.RayCastIntersect(const rayStart, rayVector: TVector;
...
function TgxDummyCube.BarycenterAbsolutePosition: TVector;
var
  i: Integer;
begin
  if Count > 0 then
...
function TgxPlane.AxisAlignedDimensionsUnscaled: TVector;
begin
...
function TgxPlane.RayCastIntersect(const rayStart, rayVector: TVector;
  intersectPoint: PVector = nil; intersectNormal: PVector = nil): Boolean;
var
...
function TgxPlane.PointDistance(const aPoint: TVector): Single;
begin
...
function TgxSprite.AxisAlignedDimensionsUnscaled: TVector;
begin
  Result.X := 0.5 * Abs(FWidth);
  Result.Y := 0.5 * Abs(FHeight);
...
begin
  ...
  if NoZWrite then
    rci.gxStates.DepthWriteMask := GLboolean(False);
...
procedure TgxPoints.SetPositions(const val: TAffineVectorList);
begin
  FPositions.Assign(val);
  StructureChanged;
end;

procedure TgxPoints.SetColors(const val: TVectorList);
begin
  FColors.Assign(val);
  StructureChanged;
end;
...
function TgxNodedLines.AxisAlignedDimensionsUnscaled: TVector;
var
  i: Integer;
begin
  RstVector(Result);
...
procedure TgxNodedLines.AddNode(const Value: TVector);
var
  n: TgxNode;
begin
  n := Nodes.Add;
...
function TgxCube.AxisAlignedDimensionsUnscaled: TVector;
begin
...
function TgxCube.RayCastIntersect(const rayStart, rayVector: TVector;
  intersectPoint: PVector = nil; intersectNormal: PVector = nil): Boolean;
var
...
procedure TgxQuadricObject.SetupQuadricParams(quadric: PGLUquadricObj);
const
...
procedure TgxQuadricObject.SetNormalQuadricOrientation(quadric: PGLUquadricObj);
const
...
procedure TgxQuadricObject.SetInvertedQuadricOrientation(quadric: PGLUquadricObj);
const
  cNormalDirectionToEnum: array [ndInside .. ndOutside] of GLEnum =
...
function TgxSphere.RayCastIntersect(const rayStart, rayVector: TVector;
  intersectPoint: PVector = nil; intersectNormal: PVector = nil): Boolean;
var
  i1, i2: TVector;
...
function TgxSphere.AxisAlignedDimensionsUnscaled: TVector;
begin
...
procedure TgxPolygonBase.AddNode(const Value: TVector);
var
  n: TgxNode;
begin
  n := Nodes.Add;
...
function TgxSuperellipsoid.RayCastIntersect(const rayStart, rayVector: TVector;
  intersectPoint: PVector = nil; intersectNormal: PVector = nil): Boolean;
var
  i1, i2: TVector;
  localStart, localVector: TVector;
...
function TgxSuperellipsoid.AxisAlignedDimensionsUnscaled: TVector;
begin
  Result.X := Abs(FRadius);
  Result.Y := Result.X;
  Result.Z := Result.X;
