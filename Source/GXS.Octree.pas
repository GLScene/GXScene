...
procedure TgxOctree.CreateTree(Depth: Integer);
begin
...
function TgxOctree.GetMidPoint(Min, Max: Single): Single;
begin
  Result := Max / 2 + Min / 2;
...
function TgxOctree.PointInNode(const Min, Max, APoint: TAffineFLTVector): BOOLEAN;
begin
...
function TgxOctree.SphereInNode(const MinExtent, MaxExtent: TAffineVector;
  const C: TVector; Radius: Single): Boolean;
...
procedure TgxOctree.WalkSphereToLeaf(Onode: POctreeNode; const P: TVector;
  Radius: Single);
begin
  Finalize(Resultarray);
...
procedure TgxOctree.WalkSphereToLeafx(Onode: POctreeNode; const P: TVector;
  Radius: Single);
var
  N: Integer;
begin
...
procedure TgxOctree.WalkRayToLeaf(Onode: POctreeNode; const P, V: TVector);
begin
  Finalize(Resultarray);
...
procedure TgxOctree.WalkRayToLeafx(Onode: POctreeNode; const P, V: TVector);
var
  N: Integer;
  Coord: TVector;
begin
...
function TgxOctree.TriIntersectNode(const MinExtent, MaxExtent, V1, V2,
...
function TgxOctree.RayCastIntersect(const RayStart, RayVector: TVector;
  IntersectPoint: PVector = nil; IntersectNormal: PVector = nil;
  TriangleInfo: POctreeTriangleInfo = nil): Boolean;
const
...
//
function TgxOctree.SphereSweepIntersect(const RayStart, RayVector: TVector;
  const Velocity, Radius: Single; IntersectPoint: PVector = nil;
  IntersectNormal: PVector = nil): Boolean;
const
...
function TgxOctree.AABBIntersect(const AABB: TAABB; M1to2, M2to1: TMatrix;
  Triangles: TAffineVectorList = nil): Boolean;
var
