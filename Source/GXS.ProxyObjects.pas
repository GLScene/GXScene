...
function TgxActorProxy.BoneMatrix(BoneIndex: integer): TMatrix;
begin
  if BoneIndex < FBonesMatrices.count then
    result := TBoneMatrixObj(FBonesMatrices.Objects[BoneIndex]).Matrix;
end;

function TgxActorProxy.BoneMatrix(BoneName: string): TMatrix;
var
  i: Integer;
begin
...
function TgxActorProxy.RayCastIntersect(const rayStart, rayVector: TVector;
