...
begin
  ...
  begin
    ...
    SetGXSceneViewer(TgxSimpleNavigation(Source).FGXSceneViewer);
    }
  ...
  begin
    if FForm.ComponentCount <> 0 then
      for I := 0 to FForm.ComponentCount - 1 do
        ...
        begin
          SetGXSceneViewer(TgxSceneViewer(FForm.Components[I]));
          Exit;
        end;
  end;
end;
...
begin
  ...
  if FGXSceneViewer <> nil then
    FGXSceneViewer.OnMouseMove := nil;
...
begin
  ...
  if FGXSceneViewer <> nil then
    if FGXSceneViewer.Camera <> nil then
      FGXSceneViewer.Camera.AdjustDistanceToTarget(
...
procedure TgxSimpleNavigation.ViewerMouseMove(Sender: TObject;
  ...
  begin
    ...
    FGXSceneViewer.Camera.MoveAroundTarget(SignX * FMoveAroundTargetSpeed * (FOldY - Y),
  ...
  begin
    ...
    FGXSceneViewer.Camera.RotateTarget(SignY * FRotateTargetSpeed * (FOldY - Y),
...
begin
  inherited;
  if (AComponent = FGXSceneViewer) and (Operation = opRemove) then
    FGXSceneViewer := nil;
...
procedure TgxSimpleNavigation.SetGXSceneViewer(
  const Value: TgxSceneViewer);
begin
  if FGXSceneViewer <> nil then
  begin
    FGXSceneViewer.RemoveFreeNotification(Self);
    FGXSceneViewer.OnMouseMove := nil;
  end;

  FGXSceneViewer := Value;

  if FGXSceneViewer <> nil then
  begin
    FGXSceneViewer.OnMouseMove := ViewerMouseMove;
    FGXSceneViewer.FreeNotification(Self);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Insert(FGXSceneViewer.FramesPerSecondText, Temp, Index);
    end;
    FForm.Caption := Temp;
    FGXSceneViewer.ResetPerformanceMonitor;
  end;
end;
