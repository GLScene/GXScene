...
type
  ...
  public
    ...
    property Items[index: Integer]: TgxSkyDomeBand read GetItems write SetItems;
    default;
...
function StarRecordColor(const starRecord : TgxStarRecord; bias : Single) : TVector;
...
begin
  if Assigned(owner) and (owner is TgxBaseSceneObject) then TgxBaseSceneObject(owner).StructureChanged;
end;
...
procedure TgxSkyDomeStars.BuildList(var rci: TgxRenderContextInfo; twinkle:
  Boolean);
var
  i, n: Integer;
  star: TgxSkyDomeStar;
...
procedure TgxSkyDomeStars.AddRandomStars(const nb: Integer; const ColorMin,
  ColorMax: TVector3b;
...
begin
  ...
  FSunDawnColor := TgxColor.CreateInitialized(Self, Vectormake(1, 0.5, 0, 0),OnColorChanged);
  FHazeColor := TgxColor.CreateInitialized(Self, VectorMake(0.9, 0.95, 1, 0),OnColorChanged);
  FSkyColor := TgxColor.CreateInitialized(Self, VectorMake(0.45, 0.6, 0.9, 0),OnColorChanged);
  FNightColor := TgxColor.CreateInitialized(Self, clrTransparent,OnColorChanged);
