...
type
  ...
  TSwizzleVector = array[0..3] of TgxTextureSwizzle;
...
const
  ...
  cTextureFormatToOpenVX: array[low(TgxInternalFormat)..high(TgxInternalFormat)] of TFormatDesc =
  (
...
function InternalFormatToOpenVXFormat(intFormat: TgxInternalFormat): Cardinal;
begin
  Result := cTextureFormatToOpenVX[intFormat].IntFmt;
end;
...
begin
  Result := tfRGBA8;
  for i := Low(cTextureFormatToOpenVX) to High(cTextureFormatToOpenVX) do
    if glFormat = cTextureFormatToOpenVX[i].IntFmt then
    begin
      Result := i;
      Exit;
    end;
...
begin
  Result := GetTextureElementSize(
    cTextureFormatToOpenVX[intFormat].ClrFmt,
    cTextureFormatToOpenVX[intFormat].DataFmt);
end;
...
begin
  ...
  begin
    Result := GL_ARB_texture_float or GL_ATI_texture_float;
    EXIT;
  end;
...
begin
  Result := cTextureFormatToOpenVX[intFormat].Flt;
end;

function IsFloatFormat(glFormat: Cardinal): boolean;
begin
  Result := IsFloatFormat(OpenVXFormatToInternalFormat(glFormat));
end;

function IsDepthFormat(intFormat: TgxInternalFormat): boolean;
begin
  Result := cTextureFormatToOpenVX[intFormat].DBit > 0;
end;

function IsDepthFormat(glFormat: Cardinal): boolean;
begin
  Result := cTextureFormatToOpenVX[OpenVXFormatToInternalFormat(glFormat)].DBit > 0;
end;

function IsCompressedFormat(intFormat: TgxInternalFormat): boolean;
begin
  Result := cTextureFormatToOpenVX[intFormat].Comp;
end;

function IsCompressedFormat(glFormat: Cardinal): boolean;
begin
  Result := cTextureFormatToOpenVX[OpenVXFormatToInternalFormat(glFormat)].Comp;
end;
