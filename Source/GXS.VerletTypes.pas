...
type
  ...
  TgxVerletDualConstraint = class(TVerletConstraint)
  private
  ...
  public
    property Items[i: Integer]: TgxVerletEdge read GetItems write SetItems; default;
  end;

  TgxVerletGlobalConstraint = class(TVerletConstraint)
  private
  ...
  TgxVerletGlobalFrictionConstraintSphere = class(TgxVerletGlobalFrictionConstraintSP)
  private
  ...
  TgxVerletGlobalFrictionConstraintBox = class(TgxVerletGlobalFrictionConstraintSP)
  private
    FCachedAABB: TAABB;
    procedure SetLocation(const Value: TAffineVector); override;
  public
  ...
  public
    property Items[i: Integer]: TgxVerletForce read GetItems write SetItems; default;
  end;
  ...
  TVFGravity = class(TgxVerletGlobalForce)
  private
  ...
  TVFAirResistance = class(TgxVerletGlobalForce)
  private
    FDragCoeff: Single;
  ...
  TVFSpring = class(TgxVerletDualForce)
  private
    FRestLength: Single;
    FStrength: Single;
    FDamping: Single;
  ...
  public
    ...
    procedure SatisfyConstraintForNode(const aNode: TVerletNode; const iteration, maxIterations: Integer); override;
    procedure SatisfyConstraintForEdge(const aEdge: TgxVerletEdge; const iteration, maxIterations: Integer); override;
    property Radius: Single read FRadius write FRadius;
  end;
  ...
  private
    ...
    FBaseSceneObject: TgxBaseSceneObject;
  ...
  function CreateVCPlaneFromGLPlane(Plane : TgxPlane; VerletWorld : TgxVerletWorld; Offset : single) : TVCFloor;
...
constructor TVerletNode.CreateOwned(const aOwner: TgxVerletWorld);
begin
...
constructor TVerletConstraint.Create(const aOwner: TgxVerletWorld);
begin
  inherited Create;
  if Assigned(aOwner) then
...
procedure TgxVerletDualConstraint.RemoveNode(const aNode: TVerletNode);
begin
...
constructor TVerletGroupConstraint.Create(const aOwner: TgxVerletWorld);
begin
...
procedure TgxVerletGlobalConstraint.AddKickbackForceAt(const Pos: TAffineVector; const Force: TAffineVector);
var
  dPos: TAffineVector;
begin
  // Sum forces
...
function TgxVerletGlobalConstraint.TranslateKickbackTorque(const TorqueCenter: TAffineVector): TAffineVector;
begin
...
procedure TgxVerletGlobalConstraint.RemoveNode(const aNode: TVerletNode);
begin
  // nothing to do here
end;
...
procedure TgxVerletGlobalConstraint.SatisfyConstraint(const iteration, maxIterations: Integer);
var
  i: Integer;
  node: TVerletNode;
begin
...
procedure TgxVerletGlobalConstraint.SatisfyConstraintForEdge(const aEdge: TgxVerletEdge;
...
constructor TgxVerletGlobalFrictionConstraint.Create(const aOwner: TgxVerletWorld);
begin
  inherited;
...
procedure TgxVerletGlobalFrictionConstraintSP.SatisfyConstraint(const iteration, maxIterations: Integer);
var
  i: Integer;
  node: TVerletNode;
  edge: TgxVerletEdge;
...
constructor TVerletGroupForce.Create(const aOwner: TgxVerletWorld);
begin
  inherited Create(aOwner);
  FNodes := TVerletNodeList.Create;
end;
...
procedure TgxVerletGlobalForce.RemoveNode(const aNode: TVerletNode);
begin
  // nothing to do here
end;
...
procedure TgxVerletDualForce.RemoveNode(const aNode: TVerletNode);
begin
  if FNodeA = aNode then
    FNodeA := nil;
  if FNodeB = aNode then
...
function TgxVerletWorld.AddNode(const aNode: TVerletNode): Integer;
begin
...
procedure TgxVerletWorld.RemoveNode(const aNode: TVerletNode);
var
  i: Integer;
begin
...
function TgxVerletWorld.AddConstraint(const aConstraint: TVerletConstraint): Integer;
begin
...
procedure TgxVerletWorld.RemoveConstraint(const aConstraint: TVerletConstraint);
begin
...
procedure TgxVerletWorld.AddSolidEdge(const aNodeA, aNodeB: TVerletNode);
var
...
function TgxVerletWorld.FirstNode: TVerletNode;
begin
  Assert(FNodes.Count > 0, 'There are no nodes in the assembly!');
  Result := FNodes[0];
end;

function TgxVerletWorld.LastNode: TVerletNode;
begin
  Assert(FNodes.Count > 0, 'There are no nodes in the assembly!');
  Result := FNodes[FNodes.Count - 1];
end;

function TgxVerletWorld.CreateOwnedNode(const Location: TAffineVector; const aRadius: Single = 0; const aWeight: Single = 1)
  : TVerletNode;
begin
...
function TgxVerletWorld.CreateStick(const aNodeA, aNodeB: TVerletNode; const Slack: Single = 0): TVCStick;
begin
...
function TgxVerletWorld.CreateSpring(const aNodeA, aNodeB: TVerletNode; const aStrength, aDamping: Single;
...
function TgxVerletWorld.CreateSlider(const aNodeA, aNodeB: TVerletNode; const aSlideDirection: TAffineVector): TVCSlider;
begin
...
constructor TVFGravity.Create(const aOwner: TgxVerletWorld);
begin
  inherited;
  FGravity.X := 0;
  FGravity.Y := -9.81;
...
constructor TVFAirResistance.Create(const aOwner: TgxVerletWorld);
begin
  inherited;
...
constructor TVCFloor.Create(const aOwner: TgxVerletWorld);
begin
  inherited;
...
procedure TVCSphere.SatisfyConstraintForEdge(const aEdge: TgxVerletEdge; const iteration, maxIterations: Integer);
var
...
// BROKEN AND VERY SLOW!
procedure TVCCube.SatisfyConstraintForEdge(const aEdge: TgxVerletEdge; const iteration, maxIterations: Integer);
var
...
procedure TVCCapsule.SatisfyConstraintForEdge(const aEdge: TgxVerletEdge; const iteration, maxIterations: Integer);
var
...
constructor TgxVerletEdge.CreateEdgeOwned(const aNodeA, aNodeB: TVerletNode);
begin
  FNodeA := aNodeA;
  FNodeB := aNodeB;
...
procedure TgxVerletWorld.CreateOctree(const OctreeMin, OctreeMax: TAffineVector; const LeafThreshold, MaxTreeDepth: Integer);
var
...
procedure TgxVerletGlobalFrictionConstraintBox.SetLocation(const Value: TAffineVector);
begin
  inherited;
...
procedure TgxVerletGlobalFrictionConstraintSphere.SetLocation(const Value: TAffineVector);
begin
  inherited;
  UpdateCachedBSphere;
end;
...
procedure TgxVerletNode.SetBaseSceneObject(
  const Value: TgxBaseSceneObject);
begin
  FBaseSceneObject := Value;

  if Assigned(VXBaseSceneObject) and NailedDown then
    FRelativePosition := AffineVectorMake(VXBaseSceneObject.AbsoluteToLocal(VectorMake(FLocation, 1)));
end;
...
begin
  if Assigned(VXBaseSceneObject) and NailedDown then
  begin
    FLocation := VXBaseSceneObject.LocalToAbsolute(FRelativePosition);
  end else
    inherited;
end;
