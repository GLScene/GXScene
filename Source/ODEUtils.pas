...
procedure ODERToGXSceneMatrix(var m: TMatrix; R: TdMatrix3; pos: TdVector3); overload;
procedure ODERToGXSceneMatrix(var m: TMatrix; R: PdMatrix3; pos: PdVector3); overload;
procedure ODERToGXSceneMatrix(var m: TMatrix; R: TdMatrix3_As3x4; pos:  TdVector3); overload;
function GXSceneMatrixToODER(m: TMatrix): TdMatrix3;
...
// Converting between GXScene and ODE formats
...
procedure CopyPosFromGeomToGX(Geom: PdxGeom; GXBaseSceneObject:  TgxBaseSceneObject);
procedure PositionSceneObject(GXBaseSceneObject: TgxBaseSceneObject; Geom: PdxGeom);
...
function CreateTriMeshFromBaseMesh(GXBaseMesh: TgxBaseMesh; Space: PdxSpace;
  var Vertices: PdVector3Array; var Indices: PdIntegerArray): PdxGeom;

function GXMatrixFromGeom(Geom: PdxGeom): TMatrix;
function GXDirectionFromGeom(Geom: PdxGeom): TVector;
function CreateODEPlaneFromGXPlane(Plane: TgxPlane; Space: PdxSpace): PdxGeom;
...
procedure ODERToGXSceneMatrix(var m: TMatrix; R: PdMatrix3; pos: PdVector3);
begin
  ODERToGXSceneMatrix(m, TdMatrix3_As3x4(R^), pos^);
end;

procedure ODERToGXSceneMatrix(var m: TMatrix; R: TdMatrix3; pos: TdVector3);
begin
  ODERToGXSceneMatrix(m, TdMatrix3_As3x4(R), pos);
end;
...
begin
  ...
  begin
    ...
    ODERToGXSceneMatrix(result, actual_R, actual_pos);
  end
  else
  begin
    ODERToGXSceneMatrix(result, R, pos);
  end;
end;
...
var
  Scale: TAffineVector;
begin
  Scale := GXBaseSceneObject.Scale.AsAffineVector;
  GXBaseSceneObject.SetMatrix(GXMatrixFromGeom(Geom));
  GXBaseSceneObject.Scale.AsAffineVector := Scale;
end;
...
var
  ...
  pos: PdVector3;
begin
  v := GXBaseSceneObject.AbsolutePosition;
  dGeomSetPosition(Geom, v.X, v.Y, v.Z);

  R := dGeomGetRotation(Geom);
  pos := dgeomGetPosition(Geom);
  m := GXBaseSceneObject.AbsoluteMatrix;
  R[0] := m.X.X;
  R[4] := m.X.Y;
  R[8] := m.X.Z;
  R[1] := m.Y.X;
...
begin
  OffsetList := nil;
  FaceExtractor := TFaceExtractor.Create(GXBaseMesh);

  try
    OffsetList := TIntegerList.Create;
    FaceExtractor.ProcessMesh;
    VertexCount := 0;
    for i := 0 to GXBaseMesh.MeshObjects.Count - 1 do
      VertexCount := VertexCount + GXBaseMesh.MeshObjects[i].Vertices.Count;
    ...
    begin
      OffsetList.Add(p);
      for j := 0 to GXBaseMesh.MeshObjects[i].Vertices.Count - 1 do
      begin
        Vertex :=
          GXBaseMesh.LocalToAbsolute(GXBaseMesh.MeshObjects[i].Vertices[j]);
    ...
    begin
      ...
      iMO := GXBaseMesh.MeshObjects.IndexOf(Face.MeshObject);
