...
uses
  ...
  Importx.OpenAL {al, alut, alTypes} ,
  GXS.VectorGeometry,
  GXS.Sound,
  GXS.Scene,
...
begin
  ...
  if error <> AL_NO_ERROR then
    raise EOpenALError.Create('OpenAL Error #' + IntToStr(error) + ' (HEX: $' +
      IntToHex(error, 4) + ')');
end;
...
begin
  ...
  alSourcefv(aSource.ManagerTag, AL_POSITION,
    PALFloat(aSource.Origin.Position.asAddress));
  CheckOpenALError;
  alSourcefv(aSource.ManagerTag, AL_DIRECTION,
    PALFloat(aSource.Origin.Direction.asAddress));
  CheckOpenALError;
  // end;
  ...
  begin
    ...
    begin
      ...
      alBufferData(aSource.Sample.ManagerTag,
        GetALFormat(aSource.Sample.sampling), aSource.Sample.Data.PCMData,
  ...
  begin
    ...
    CheckOpenALError;
    alSourcef(aSource.ManagerTag, AL_CONE_OUTER_ANGLE,
      aSource.OutsideConeAngle);
    CheckOpenALError;
    alSourcef(aSource.ManagerTag, AL_CONE_OUTER_GAIN,
      aSource.ConeOutsideVolume);
  end;
